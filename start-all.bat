...
start "LyntFlow - Editor (%EDITOR_PORT%)" cmd /k "cd /d "%EDITOR_PATH%" && echo ===== EDITOR - Porta %EDITOR_PORT% (/editor) ===== && pnpm dev"
timeout /t 3 /nobreak >nul

:: Iniciar CMS (por ultimo, pois e o servidor principal)
echo [CMS] Iniciando CMS na porta %CMS_PORT% (Servidor Principal)...
start "LyntFlow - CMS (%CMS_PORT%)" cmd /k "cd /d "%CMS_PATH%" && echo ===== CMS - Porta %CMS_PORT% (Servidor Principal) ===== && pnpm dev"
