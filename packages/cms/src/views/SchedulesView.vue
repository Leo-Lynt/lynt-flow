...
<template>
  <AppLayout>
    ...
    <div v-else-if="!hasSchedules" class="text-center py-12 glass-card backdrop-blur-xl bg-white/30 rounded-lg border border-white/20">
    ...
    <div v-else>
      <div class="space-y-4">
        <div
          ...
          :key="schedule._id"
          class="glass-card backdrop-blur-xl bg-white/30 rounded-lg border border-white/20 p-6 hover:shadow-xl hover:bg-white/40 transition-all"
        >
            ...
            <div class="flex-1">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                ...
                <div class="flex items-center text-sm">
                  <Icon icon="lucide:calendar-clock" class="w-4 h-4 text-brand-purple mr-2" />
                  <div>
                    ...
                    <div v-if="schedule.enabled && schedule.nextExecutionAt" class="text-xs text-brand-purple">
                ...
                <!-- Execution Stats -->
                <div class="flex items-center text-sm">
                  <Icon icon="lucide:activity" class="w-4 h-4 text-brand-green mr-2" />
                  <div>
                    ...
                    <div
                      ...
                      :class="[
                        'text-xs',
                        schedule.lastExecutionStatus === 'success' ? 'text-brand-green' : 'text-brand-red'
                      ]"
                    >
                ...
                <div v-if="schedule.consecutiveFailures > 0" class="flex items-center text-sm">
                  <Icon icon="lucide:alert-triangle" class="w-4 h-4 text-brand-red mr-2" />
                  <div>
                    <div class="text-gray-600 text-xs">Falhas consecutivas:</div>
                    <div class="font-medium text-brand-red">{{ schedule.consecutiveFailures }}</div>
                  </div>
                </div>
              </div>

              <!-- Paused Reason -->
              <div v-if="schedule.pausedReason" class="bg-brand-orange/10 border border-brand-orange/30 rounded p-3 mb-3 backdrop-blur-sm">
                <div class="flex items-start">
                  <Icon icon="lucide:alert-circle" class="w-4 h-4 text-brand-orange mr-2 mt-0.5" />
                  <div class="text-sm text-brand-orange">
          ...
          <div class="flex items-center space-x-2 ml-4">
            <button
              ...
              :class="[
                ...
                schedule.enabled
                  ? 'text-brand-orange hover:bg-white/30'
                  : 'text-brand-green hover:bg-white/30'
              ]"
            >
              <Icon
            ...
            <button
              ...
              title="Ver Execuções"
              class="p-2 text-brand-purple hover:bg-white/30 rounded transition-all"
            >
            ...
            <button
              ...
              title="Excluir"
              class="p-2 text-brand-red hover:bg-white/30 rounded transition-all"
            >
      ...
      <!-- Pagination -->
      <div v-if="totalPages > 1" class="mt-6 flex items-center justify-between glass-card backdrop-blur-xl bg-white/30 rounded-lg border border-white/20 p-4">
        ...
        <div class="flex items-center space-x-2">
          <button
            ...
            :class="[
              'px-3 py-2 rounded transition-all text-sm font-medium tracking-wide',
              currentPage === 1
                ? 'text-gray-400 cursor-not-allowed'
                : 'text-brand-purple hover:bg-white/40'
            ]"
          >
          ...
          <div class="flex items-center space-x-1">
            <button
              ...
              @click="goToPage(page)"
              :class="[
                'px-3 py-1.5 rounded text-sm font-medium tracking-wide transition-all',
                page === currentPage
    ...
    >
      ...
      >
        <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 class="text-xl font-bold text-gray-900 tracking-wide">Criar Agendamento</h2>
          <button
            @click="showCreateModal = false"
            class="text-gray-500 hover:text-gray-700 hover:bg-gray-100 p-1 rounded transition-all"
          >
