/**
 * Node Catalog Loader - FRONTEND VERSION
 *
 * ⚠️ DUPLICAÇÃO JUSTIFICADA
 * Este arquivo existe em core/ e frontend/ por dependência do registry local.
 *
 * - CORE: Usa registry.js do core (métodos puros)
 * - FRONTEND: Usa registry.js do frontend (métodos core + browser)
 *
 * RAZÃO: loader.js importa registry.js local (linha 6), e cada ambiente
 * precisa do seu próprio registry com estratégias diferentes de module loading.
 *
...
export async function loadNodeCatalog(apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:3001') {
  ...
  try {
    ...
    // API retorna {success, data, version, cached}
